...
import { clusterSessions, clusterTotalRevenue, type SessionCluster } from '../../utils/groups';
...
export default function Sidebar({ onCloseMobile }: SidebarProps) {
  ...
  // Group multi-stall event days into one entry; a cluster counts as active
  // while any of its stalls is still live.
  const clusters = clusterSessions(sessions);
  const activeClusters = clusters.filter(c => c.sessions.some(s => s.status === 'active'));
  const closedClusters = clusters.filter(c => c.sessions.every(s => s.status === 'closed'));
      ...
      <div className="flex-1 overflow-y-auto px-2">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-0.5">
              {activeClusters.map((c, i) => (
                ...
                >
                  <ClusterItem cluster={c} onClick={handleSessionClick} />
                </motion.div>
              ))}
            </div>
          </div>
        ...
        {closedClusters.length > 0 && (
          <div className="mb-4">
            ...
            <div className="space-y-0.5">
              {closedClusters.map(c => (
                <ClusterItem key={c.key} cluster={c} onClick={handleSessionClick} />
              ))}
            </div>
          </div>
        )}
  ...
  // A cluster is either a lone stall (plain row) or an event-day group: one
  // card holding every stall from that day, each keeping its own stats.
  function ClusterItem({ cluster, onClick }: { cluster: SessionCluster; onClick: () => void }) {
    if (!cluster.group) {
      return <SessionItem session={cluster.sessions[0]} onClick={onClick} />;
    }
    const total = clusterTotalRevenue(cluster);
    return (
      <div className="rounded-xl border border-gold/20 bg-gold/[0.04] p-1">
        <div className="flex items-center justify-between gap-2 px-3 pt-1.5 pb-1">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-semibold text-sm text-gold truncate">{cluster.group.name}</span>
            </div>
            <span className="text-xs text-gray-600">
              {cluster.sessions.length} stalls &middot; {formatDate(cluster.group.date)}
            </span>
          </div>
        ...
        <div className="space-y-0.5">
          {cluster.sessions.map(s => (
            <SessionItem key={s.id} session={s} onClick={onClick} />
          ))}
        </div>
      </div>
    );
